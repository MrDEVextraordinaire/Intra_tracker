#!/usr/bin/env python3
"""Test merge logic without dtale."""
import json
import pandas as pd
...
def build_df(raw=None, users=None):
    ...
    for login in users:
        ...
        if valid_p: row["piscine_avg"] = round(sum(row[k] * PW[k] for k in valid_p) / sum(PW[k] for k in valid_p), 4)
        ...
        for k in RANK_KEYS:
            ...
            rank_results[k] = r
            if r: row[f"{k}_score"] = r["final"]
        
        valid_ranks = [k for k in RANK_KEYS if rank_results.get(k)]
        if valid_ranks: row["rank_score"] = round(sum(rank_results[k]["final"] * RANK_WEIGHTS.get(k, 1) for k in valid_ranks) / sum(RANK_WEIGHTS.get(k, 1) for k in valid_ranks), 4)
        else: row["rank_score"] = None
...
# Test merge
df1 = build_df()
print(f"Initial: {df1.shape[0]} users")

# Simulate polling - add new data
with open("data/exam_ranks.json") as f:
    raw = json.load(f)
# Add a fake new user
raw["test_new_user"] = raw.get("kraghib", {}).copy()

df2 = build_df(raw, ["test_new_user"])
print(f"New user only: {df2.shape[0]} users")

# Merge logic - should keep latest (df2) for duplicates
updated = pd.concat([df1.reset_index(), df2.reset_index()]).set_index("user")
print(f"After merge: {updated.shape[0]} users")

# Check duplicates
dupes = updated.index[updated.index.duplicated()]
print(f"Duplicates: {len(dupes)}")
if len(dupes) > 0:
    print(f"Dupe list: {list(dupes)}")
    # Test that we keep latest
    print(f"kraghib final_score: {updated.loc['kraghib', 'final_score']}")
    # Verify merge uses drop_duplicates
    updated_clean = updated.loc[~updated.index.duplicated(keep='last')]
    print(f"After dedup: {updated_clean.shape[0]} users")